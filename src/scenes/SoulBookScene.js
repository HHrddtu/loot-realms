...
import { getAllSoulEntries, getNextSoulLevelInfo } from '../soulBook.js';
import { GAME_WIDTH, GAME_HEIGHT, SOUL_BOOK_LEVELS, BESTIARY_DIFF_MULT } from '../config/index.js';
import { t, getLang, translateName } from '../i18n.js';

export default class SoulBookScene extends Phaser.Scene {
    constructor() {
        super('SoulBook');
    }

    init(data) {
        this.classKey = data.classKey || 'angel';
    ...
    create() {
        ...
        this.entries = getAllSoulEntries();
        ...
        this.add.text(200, 22, t('soulBook.title'), {
            fontSize: '26px', fill: '#ecf0f1', fontFamily: 'Georgia', fontStyle: 'bold'
        }).setOrigin(0.5);

        this.add.text(200, 52, `${t('soulBook.collected')}: ${progress.current}/${progress.total}`, {
            fontSize: '14px', fill: '#aaa', fontFamily: 'Arial'
        }).setOrigin(0.5);

        this.add.text(200, 68, 'Defeat enemies to collect their souls. Purify them to gain +HP, +DMG, faster corruption decay.', {
    ...
    _createGrid() {
        ...
        this.entries.forEach((entry, i) => {
            ...
            const bg = this.add.rectangle(x + itemW / 2, y + itemH / 2, itemW, itemH, isSeen ? 0x0a1a2a : 0x111118)
                .setStrokeStyle(1, isSeen ? 0x224466 : 0x222233)
                .setInteractive({ useHandCursor: true });

            if (isSeen && entry.texKey) {
                try {
                    const spr = this.add.sprite(x + itemW / 2, y + 18, entry.texKey).setDisplaySize(28, 28);
                    spr.setTint(0xaaccff);
                } catch (e) {}
            } else {
            ...
            this.add.text(x + itemW / 2, y + 44, isSeen ? translateName(entry) : t('soulBook.locked'), {
    ...
    _createDetailPanel() {
        ...
        this.detailBg = this.add.rectangle(px + pw / 2, py + ph / 2, pw, ph, 0x080f18)
            .setStrokeStyle(1, 0x224466).setDepth(5);

        this.detailEmptyText = this.add.text(px + pw / 2, py + ph / 2, 'Select a soul to view details\n\nDefeat enemies to collect their souls.\nPurify them to gain passive bonuses:\n+HP, +DMG, faster corruption decay\n\nEach enemy type has unique weakness\nand essence to study.', {
        ...
        this.detailTitle = this.add.text(px + 15, py + 12, '', {
            fontSize: '24px', fill: '#ecf0f1', fontFamily: 'Georgia', fontStyle: 'bold'
        }).setVisible(false).setDepth(6);

        this.detailLevel = this.add.text(px + 15, py + 42, '', {
            fontSize: '13px', fill: '#3498db', fontFamily: 'Arial'
        }).setVisible(false).setDepth(6);
        ...
        this.detailWeakness = this.add.text(px + 15, py + 220, '', {
            fontSize: '12px', fill: '#e74c3c', fontFamily: 'Arial',
            wordWrap: { width: pw - 30 }, lineSpacing: 4
        }).setVisible(false).setDepth(6);

        this.detailEssence = this.add.text(px + 15, py + 300, '', {
            fontSize: '12px', fill: '#9b59b6', fontFamily: 'Arial',
            wordWrap: { width: pw - 30 }, lineSpacing: 4
        }).setVisible(false).setDepth(6);

        this.detailPurify = this.add.text(px + 15, py + 380, '', {
            fontSize: '12px', fill: '#2ecc71', fontFamily: 'Arial',
            wordWrap: { width: pw - 30 }, lineSpacing: 4
        }).setVisible(false).setDepth(6);

        this.detailLore = this.add.text(px + 15, py + 445, '', {
    ...
    _clearDetail() {
        ...
        this.detailEssence.setVisible(false);
    ...
    _selectEntry(index) {
        ...
        this.listItems.forEach((item, i) => {
            item.bg.setStrokeStyle(i === index ? 2 : 1, i === index ? 0xf1c40f : 0x224466);
        });
        ...
        const lvlNames = ['Empty', 'Touched', 'Tuned', 'Resonant', 'Attuned', 'Harmonized'];
        const lvlText = lvlNames[entry.level] || 'Unknown';
        const cfg = SOUL_BOOK_LEVELS.find(l => l.level === entry.level) || SOUL_BOOK_LEVELS[0];
        const hpB = Math.floor(cfg.hpBonus * diffMult);
        const dmB = Math.floor(cfg.dmgBonus * diffMult);
        const cdB = (cfg.corDecay * diffMult).toFixed(3);
        const bonusText = hpB > 0 || dmB > 0 || cdB > 0 ? ` (+${hpB} HP, +${dmB} DMG, -${cdB} COR/s)` : '';
        this.detailLevel.setText(`Purification: ${entry.level}/5 — ${lvlText}${bonusText}`).setVisible(true);
        this.detailLevel.setPosition(px + 15, py + 38);

        this.detailStats.setText(`Biome: ${entry.biome}\nSouls collected: ${entry.collected}\nDifficulty: ${this.difficulty} (×${diffMult})`).setVisible(true);
        this.detailStats.setPosition(px + 15, py + 56);

        if (entry.texKey) {
            try {
                const spr = this.add.sprite(px + pw / 2, py + 95, entry.texKey)
                    .setDisplaySize(48, 48).setDepth(7);
                spr.setTint(0xaaddff);
                this.previewSprite = spr;
            } catch (e) {}
        }

        const nextInfo = getNextSoulLevelInfo(entry.key);
        if (nextInfo) {
            const barX = px + 15;
            const barY = py + 120;
            const barW = pw - 30;
            ...
            const barText = this.add.text(barX + barW / 2, barY + barH / 2, `${nextInfo.currentCount}/${nextInfo.requiredCount} souls → Lv.${nextInfo.nextLevel}`, {
                fontSize: '10px', fill: '#fff', fontFamily: 'Arial', fontStyle: 'bold'
            }).setOrigin(0.5).setDepth(8);
            this.detailElements.push(barBg, barFill, barText);
        } else {
            const maxBar = this.add.text(px + pw / 2, py + 125, '★ MAX PURIFICATION ★', {
                fontSize: '12px', fill: '#f1c40f', fontFamily: 'Arial', fontStyle: 'bold'
            }).setOrigin(0.5).setDepth(7);
            this.detailElements.push(maxBar);
        }

        const lvlLines = SOUL_BOOK_LEVELS.slice(1).map(lv => {
            const hp = Math.floor(lv.hpBonus * diffMult);
            const dm = Math.floor(lv.dmgBonus * diffMult);
            const cd = (lv.corDecay * diffMult).toFixed(3);
            const unlocked = entry.level >= lv.level;
            return `${unlocked ? '✓' : '○'} Lv.${lv.level} (${lv.countRequired}×): +${hp}HP +${dm}DMG -${cd}COR/s`;
        }).join('\n');
        ...
        if (entry.weakness) {
            this.detailWeakness.setText(t('soulBook.weakness') + ':\n' + entry.weakness).setVisible(true);
            this.detailWeakness.setPosition(px + 15, py + 310);
        }

        if (entry.essence) {
            this.detailEssence.setText(t('soulBook.essence') + ':\n' + entry.essence).setVisible(true);
            this.detailEssence.setPosition(px + 15, py + 370);
        }

        if (entry.purification) {
            this.detailPurify.setText(t('soulBook.purification') + ':\n' + entry.purification).setVisible(true);
            this.detailPurify.setPosition(px + 15, py + 430);
        }
